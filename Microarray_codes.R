...
filter_table_mca <- function(df_tables_info_mca,
                             year_list=c(),
                             author_list=c(),
                             tumor_list=c(),
                             tissue_list=c(),
  ...
  if(length(year_list)>0 ){
    df_filter_year <- filter(df_tables_info_mca,  Year %in% year_list)
    final_df <- rbind(final_df,df_filter_year)
  }
  
  if(length(author_list)>0 ){
    df_filter_author <- filter(df_tables_info_mca,  Autor %in% author_list)
    final_df <- rbind(final_df,df_filter_author)
  }
  
  if(length(gse_code_list)>0 ){
    df_filter_gse <- filter(df_tables_info_mca,  Code_GSE %in% gse_code_list)
    final_df <- rbind(final_df,df_filter_gse)
  }
  
  if(length(gender_list)>0 ){
    df_filter_gender <- filter(df_tables_info_mca,  Sex %in% gender_list)
    final_df <- rbind(final_df,df_filter_gender)
  }
  
  if(length(time_days_list)>0 ){
    df_filter_time_days <- filter(df_tables_info_mca,  Time_days %in% time_days_list)
    final_df <- rbind(final_df,df_filter_time_days)
  }
  
  if(length(tumor_list)>0 ){
    df_filter_tumor <- filter(df_tables_info_mca,  Tumor %in% tumor_list)
    final_df <- rbind(final_df,df_filter_tumor)
  }
  
  if(length(tissue_list)>0 ){
    df_filter_tissue <- filter(df_tables_info_mca,  Tissue %in% tissue_list)
    final_df <- rbind(final_df,df_filter_tissue)
  }
  
  if(length(tissue_names_list)>0 ){
    df_filter_tissue_name <- filter(df_tables_info_mca,  Tissue_name %in% tissue_names_list)
    final_df <- rbind(final_df,df_filter_tissue_name)
  }
  
  if(length(pmid_code_list)>0 ){
    df_filter_pmid <- filter(df_tables_info_mca,  PMID %in% pmid_code_list)
